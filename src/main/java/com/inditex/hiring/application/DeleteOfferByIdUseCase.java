...
import com.inditex.hiring.domain.command.handler.DeleteOfferByIdCommandHandler;
...
public class DeleteOfferByIdUseCase implements UseCase<DeleteOfferByIdCommand, Void> {

	private final DeleteOfferByIdCommandHandler handler;

	@Override
	public Void execute(DeleteOfferByIdCommand input) {

		return Try.of(() -> handler.handle(input))
		          .getOrElseThrow(throwable -> new TechnicalException(HttpStatus.SERVICE_UNAVAILABLE.value(),
				          "Error deleting offer by id",
